...
//        calibrationEnd.setOnClickListener(this);
		...
		if (TextUtils.isEmpty(vocPath) || TextUtils.isEmpty(calibrationPath)) {
			...
			Toast.makeText(ORBSLAMForTestActivity.this, "init has been started!", Toast.LENGTH_LONG).show();
		...
		switch (v.getId()) {
			...
			case R.id.calibrationStart:
				if (!calibrationIsStart) {
					...
					System.arraycopy(pos, 0, caliStartPos, 0, 3);
					Toast.makeText(this, "Calibration Start", Toast.LENGTH_LONG).show();
				} else if (calibrationIsStart) {

					calibrationIsStart = false;
					calibrationStart.setText("Start Calibration");
					System.arraycopy(pos, 0, caliEndPos, 0, 3);
					...
					Toast.makeText(this, "Calibration End", Toast.LENGTH_LONG).show();
				}
				break;
			case R.id.distanceStart:
				if (!distanceIsStart && !distanceIsEnd) {
					...
					System.arraycopy(pos, 0, distStartPos, 0, 3);
					Toast.makeText(this, "Distance Calculation Start", Toast.LENGTH_LONG).show();
				} else if (distanceIsStart && !distanceIsEnd) {
					distanceIsStart = false;
					distanceIsEnd = true;
					distanceStart.setText("No Distance");
					System.arraycopy(pos, 0, distEndPos, 0, 3);
					...
					distTextView.setText("Moving Distance: " + displacement + "m");

					Toast.makeText(this, "Distance Calculation End", Toast.LENGTH_LONG).show();
				} else if (!distanceIsStart && distanceIsEnd) {
					distanceIsStart = false;
					distanceIsEnd = false;
					...
					new Thread(new Runnable() {
						...
						public void run() {
							while (isSLAMRunning) {
								...
								// esultImg.setPixels(resultInt, 0, w, 0, 0, w, h);
...
//        }
	...
	public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
		switch (requestCode) {
			case 10:
				checkPermission = grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED;
		}
	}
